...
// ── 0. Parse Environment for Baseline Bootstrap ─────────────────────────────
...
(async () => {
  ...
  const ENV_CONFIG = {
    ...
    preprod: {
      baseDir: '/data/am-repos/am-infra-automation/terraform',
      bin: 'terraform',
      isRemote: true,
      envName: 'preprod',
      sshHost: 'root@103.127.146.57',
      sshPort: 7576,
      localBase: TF_DIR,
      remoteBase: '/data/am-repos/am-infra-automation',
  ...
  if (command === 'init') {
    ...
    for (let i = 0; i < extraArgs.length; i++) {
      ...
      if (arg.startsWith('-p') || arg.startsWith('--package') || arg.startsWith('--pkg')) {
        if (!arg.includes('=')) i++;
      } else if (arg.startsWith('-e') || arg.startsWith('--env')) {
        if (!arg.includes('=')) i++;
      } else if (arg.startsWith('-a') || arg.startsWith('--apps')) {
        if (!arg.includes('=')) i++;
  ...
  function startAuthentikTunnelGuardian() {
    let current = null;
    let stopped = false;

    function spawnTunnel() {
      if (stopped) return;
      const kubeconfigPath = path.join(config.stateRoot, `am-${config.envName}-config`).replace(/\\/g, '/');
      const proc = spawn(
        'kubectl',
        ['--kubeconfig', kubeconfigPath, 'port-forward', 'service/authentik-server', '9001:80', '-n', 'identity'],
        { shell: true, env: process.env }
      );
      current = proc;
      proc.stderr.on('data', () => { });
    ...
    probeTcp(9001, (alreadyUp) => {
      if (!alreadyUp && !stopped) {
        console.log(`🔑 [AUTHENTIK] Starting self-healing Authentik port-forward (9001:80)...`);
        spawnTunnel();
      }
    });
    ...
    return new Promise((resolve) => {
      ...
      function check() {
        ...
        const req = http.get('http://127.0.0.1:9001/', (res) => {
          if (res.statusCode >= 200 && res.statusCode < 400) {
            console.log(`✅ [AUTHENTIK] Authentik server is responsive (HTTP ${res.statusCode}).`);
            resolve(true);
          } else {
            console.log(`⏳ [AUTHENTIK] Authentik starting up, HTTP status ${res.statusCode}... (attempt ${attempt}/${maxAttempts})`);
            scheduleRetry();
          }
        });
        ...
        req.setTimeout(3000);
        req.on('error', () => {
          console.log(`⏳ [AUTHENTIK] Authentik not yet reachable (attempt ${attempt}/${maxAttempts})...`);
          scheduleRetry();
        });
        req.on('timeout', () => { req.destroy(); });
      }
      ...
      function scheduleRetry() {
        if (attempt >= maxAttempts) {
          console.error(`❌ [AUTHENTIK] Timeout waiting for Authentik to become ready.`);
          process.exit(1);
        }
  ...
  function ensureAuthentikReady(callback) {
    console.log(`\n🔌 [AUTHENTIK] Starting local Authentik port-forward to check readiness...`);
    const guardian = startAuthentikTunnelGuardian();
    
    setTimeout(() => {
      pollAuthentikHealth()
        .then(() => {
          console.log(`✅ [AUTHENTIK] Authentik is fully ready. Proceeding to final configuration.`);
          callback(guardian);
        })
        .catch((err) => {
          console.error(`\n❌ [AUTHENTIK] ${err.message}`);
          guardian.stop();
          process.exit(1);
        });
    }, 5000);
  ...
  async function execute() {
    ...
    const isIdentityTwoPhase = pkg === 'identity' && ['apply', 'plan'].includes(command) && targets.length === 0;
    ...
    try {
      ...
      if (isFoundationTwoPhase) {
      ...
      } else if (isIdentityTwoPhase) {
        console.log(`\n📦 [IDENTITY] Phase 1 — Deploying core Authentik infrastructure (Postgres, Redis, Helm)...`);
        const identityCoreTargets = [
          'module.identity.module.authentik_core.helm_release.authentik',
          'module.identity.module.authentik_core.kubernetes_stateful_set.postgres',
          'module.identity.module.authentik_core.kubernetes_persistent_volume_claim.postgres_data',
          'module.identity.module.authentik_core.kubernetes_service.postgres',
          'module.identity.module.authentik_core.kubernetes_service.postgres_exposer',
          'module.identity.module.authentik_core.kubernetes_deployment.redis',
          'module.identity.module.authentik_core.kubernetes_persistent_volume_claim.redis_data',
          'module.identity.module.authentik_core.kubernetes_service.redis'
        ];
        const phase1Args = [...opArgs];
        identityCoreTargets.forEach(t => {
          phase1Args.push('-target', t);
        });
        if (command !== 'plan') {
          phase1Args.push('-auto-approve');
        }
        const phase1 = await runTerraform(config, command, phase1Args);
        if (phase1.code !== 0) {
          console.error(`\n❌ [IDENTITY Phase 1] failed (exit ${phase1.code}).`);
          process.exit(phase1.code);
        }
        console.log(`\n✅ [IDENTITY Phase 1] Core Authentik infrastructure deployed.`);

        if (command !== 'plan') {
          console.log(`\n🔐 [IDENTITY] Phase 2 — Waiting for Authentik to migrate database & become ready...`);
          const activeAuthentikTunnel = await new Promise((resolve) => {
            ensureAuthentikReady((tunnel) => {
              resolve(tunnel);
            });
          });

          console.log(`\n📦 [IDENTITY] Phase 2 — Applying users, groups, and DB roles...`);
          const phase2Args = [...opArgs];
          if (command !== 'plan') {
            phase2Args.push('-auto-approve');
          }
          const phase2 = await runTerraform(config, command, phase2Args);

          if (activeAuthentikTunnel && activeAuthentikTunnel.stop) {
            console.log(`🔌 [TUNNEL] Shutting down Authentik tunnel guardian...`);
            activeAuthentikTunnel.stop();
          }

          if (phase2.code !== 0) {
            console.error(`\n❌ [IDENTITY Phase 2] failed (exit ${phase2.code}).`);
          } else {
            console.log(`\n✅ [${command.toUpperCase()}] completed successfully.`);
          }
          process.exit(phase2.code);
        ...
        const vaultModuleArgs = [...opArgs, '-target', 'module.vault', '-var="vault_root_token=bootstrap"'];
        if (command !== 'plan') {
          vaultModuleArgs.push('-auto-approve');
        }
        ...
        if (!isVaultPkg) {
          await syncFromVault(BASE_DIR, env);
        }

        if (pkg === 'access') {
          authentikTunnel = startAuthentikTunnelGuardian();
        }

        // Execute requested command
        const mainResult = await runTerraform(config, command, [...tfArgs, ...opArgs]);

        if (authentikTunnel && authentikTunnel.stop) {
          authentikTunnel.stop();
        }
